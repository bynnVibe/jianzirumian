...
对问答主链路跑离线回归，评估框架仿照 Ragas，量化「忠实度 / 回答相关性 / 上下文精确率 / 上下文召回率」四项核心指标，并保留检索命中率与 judge 对照分，防止 prompt、检索、重排等优化引入回退。入口：侧边栏「回归评测」（仅管理员可见，路由 `/eval`）。
...
| **run 级指标** | Ragas 四指标均值：忠实度（答案陈述可由检索上下文推导的比例）、回答相关性（LLM 由答案反推问题与原问题的嵌入余弦相似度）、上下文精确率（逐片段相关性判定的加权 precision@k）、上下文召回率（参考答案陈述被检索覆盖的比例）；另含检索命中率、平均 judge 分、平均耗时 |
| **运行对比** | 运行历史列表 + 用例结果明细（每条用例的四项 Ragas 得分/耗时/错误，展开可看逐条陈述与片段判定依据），跨 run 对比指标变化判断是否回退 |
