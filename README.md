...
| 功能 | 说明 |
|------|------|
| **图片上传** | 支持 JPG、PNG、BMP、TIFF 等常见图片格式，可拖拽或点击上传 |
| **Word / PDF 导入** | 上传 .doc/.docx/.pdf 文件，按页提取文字，可逐页编辑、勾选后入库；表格自动解析为 Markdown 格式保留结构 |
| **多引擎 OCR** | 支持多种 OCR 引擎，用户可在上传界面自由切换 |
| **预览确认 + 编辑润色** | 先预览识别结果，可直接编辑或一键 AI 润色，**保存的是最终编辑后的文本** |
| **图片预览操作** | 长按拖动、缩放、旋转，方便核对识别结果 |
| **自定义保存标题** | 上传时可自定义知识文档的标题（默认自动填充文件名），便于后续按标题精准搜索 |
| **目标知识库选择** | 两级筛选：先选公共/个人，再选具体知识库；支持新建个人知识库 |
| **自动向量化** | 文本自动分割 → Embedding 向量化 → 存入 FAISS 向量库 |
...
- 总量统计：提问次数、知识库检索次数、联网搜索次数、上传笔记数、Token 消耗、活跃天数、会话数、收藏数、个人知识库数、点赞/点踩
...
# 后端依赖（含 FAISS / PaddleOCR / LangChain，体积较大，建议国内配 pip 镜像）
cd backend
...
- [ ] 上传一张手写笔记照片 → OCR 识别出文字 → 保存到知识库 → 回到对话页提问，能得到带来源引用的回答
...
```
上传笔记 → 输入保存标题（可选，默认文件名）→ 选择目标知识库（公共/个人 → 具体库）
→ 上传图片/文档 → OCR识别/按页提取（Word/PDF 表格自动保留为 Markdown 格式）
→ 预览编辑/一键润色 → 保存到知识库 → 文本分割 → 向量化 → FAISS 存储
→ （后台）llm-wiki 知识编译：自动提炼为结构化百科词条（可开关）
```
